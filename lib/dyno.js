...
class Dyno {
  ...
  */
  constructor (opts) {
    ...
    this.dataBuffer = ''
    this.dataBufferLength = 30 // 'heroku-command-exit-status 256'.length
  ...
  attach () {
    return new Promise((resolve, reject) => {
      ...
      c.on('close', () => {
        process.stdout.write(this.dataBuffer)
        this.dataBuffer = ''
  ...
  _readData (c) {
    ...
    return data => {
      ...
      let exitCode = this.dataBuffer.match(/heroku-command-exit-status (\d+)/m)
      if (exitCode) {
        process.stdout.write(this.dataBuffer.replace(/^heroku-command-exit-status \d+$\n?/m, ''))
        this.dataBuffer = ''
