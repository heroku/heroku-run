  ...
  _handle (localServer) {
    ...
    if (this.opts.listen) {
      ...
      sshProc.stderr.on('data', (data) => {
        ...
        if (this._isDebug() || (data.indexOf("Warning: Permanently added '[127.0.0.1]") === -1 && data.indexOf("Permission denied (publickey).") === -1)) {
           process.stderr.write(data)
        }
      })
      sshProc.on('close', (code, signal) => {
        // there was a problem connecting with the ssh key
        if (lastErr.length > 0 && lastErr.indexOf('Permission denied') !== -1) {
